...
public class ProductService {
	...
	public ProductSaveResponse enroll(User user, ProductSaveRequest productSaveRequest, List<MultipartFile> files) {
		Preconditions.checkArgument(productSaveRequest.getProductName().isEmpty(),
			"상품명은 필수 값입니다.");
		Preconditions.checkArgument(productSaveRequest.getDescription().isEmpty(),
			"상품 설명은 필수 값입니다.");
		...
		if (!files.isEmpty()) {
			if (files.size() == 1) {
				ProductImage productImage = fileService.uploadImage(user.getUserId(), files.get(0));
				productImage.mappingProduct(product);
			} else {
		...
		return new ProductSaveResponse().from(product);
	}

	public ProductFindResponse findById(Long id) {
		Product product = productRepository.findById(id)
			.orElseThrow(() -> new NotFoundProductException(id));
		UserResponse userResponse = new UserResponse().from(product.getUser());
		List<ImageResponse> imageResponses = fileService.loadImages(product);
		ProductSaveResponse productSaveResponse = new ProductSaveResponse().from(product);
		return new ProductFindResponse().from(userResponse, imageResponses, productSaveResponse);
	}

	public ProductUpdateResponse update(User user, Long productId, ProductUpdateRequest productUpdateRequest,
		...
		return new ProductUpdateResponse().from(product.getId());
	}
	...
	public ProductDeleteResponse delete(Long productId) {
		...
		return new ProductDeleteResponse().from(product.getId());
	}
